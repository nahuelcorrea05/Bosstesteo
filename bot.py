...
def save_bosses():
    try:
        ...
        print(f"✅ Bosses guardados en {SAVE_FILE}")
    except Exception as e:
        print(f"❌ Error al guardar bosses: {e}")
        traceback.print_exc()

def load_bosses():
    try:
        if not os.path.exists(SAVE_FILE):
            print(f"⚠️ {SAVE_FILE} no existe, creando archivo vacío")
            save_bosses()
            return
        ...
        print(f"✅ {len(bosses)} bosses cargados")
    except Exception as e:
        print(f"❌ Error al cargar bosses: {e}")
        traceback.print_exc()
    ...
    for guild in bot.guilds:
        for ch in guild.text_channels:
            if ch.permissions_for(guild.me).send_messages:
                channel = ch
                print(f"✅ Canal de alertas configurado: {ch.name}")
                break
        if channel:
            break
    
    if not channel:
        print("⚠️ No se encontró canal para enviar alertas")
            ...
            for name, data in bosses.items():
                ...
                if (remaining <= 60 and remaining > 0 
                    ...
                    try:
                        ...
                        print(f"❌ Error enviando alerta 1min: {e}")
                ...
                if (remaining <= 0 and name not in spawn_announced):
                    ...
                    try:
                        ...
                        print(f"❌ Error enviando alerta spawn: {e}")
